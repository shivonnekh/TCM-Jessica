...
-- Column migrations for existing tables: see _migrate_pg_user_columns()
-- in repo_pg.py — done in Python because asyncpg 0.31 + Python 3.14
-- chokes on ALTER TABLE ADD COLUMN IF NOT EXISTS (NoneType decode bug
-- in simple-query protocol path). Code-level migration uses
-- information_schema lookup + conditional ADD COLUMN — same effect,
