...
class InferredPattern(BaseModel):
    """A 證 (TCM pattern/syndrome) the Planner inferred from the user's
    message + recent history + known constitution.

    Note: this is an OBSERVATION not a DIAGNOSIS. The clinic doctor
    confirms / rejects via /admin. Writer surfaces with "可能屬於..."
    advisory language only when confidence >= 0.7.
    """

    model_config = ConfigDict(frozen=True)

    name: str               # e.g. "肝鬱氣滯", "心脾兩虛"
    confidence: float       # 0.0-1.0
    evidence: list[str] = Field(default_factory=list)
    # Plain-language explanation Writer can paraphrase (e.g.
    # "情緒易繃緊、心煩、容易胸悶嘅狀態"). Generated by Planner LLM.
    layman_zh: str = ""
...
class PlannerDecision(BaseModel):
    ...
    Fields:
      ...
      - inferred_patterns: TCM 證型 (pattern) inferences with confidence.
        Pipeline appends to user.observed_patterns (append-only). Writer
        surfaces only when confidence >= 0.7 with advisory language.
        ADVISORY ONLY — clinic doctor confirms/rejects via /admin.
    """
    ...
    inferred_patterns: list[InferredPattern] = Field(default_factory=list)
